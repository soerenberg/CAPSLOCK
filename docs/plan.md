...
- `id` A unique integer id to avoid collisions. In the initial set up this could be equal to the (unique) rank of the country's name with respect to lexicographical order.
...
- `groups` This should be a list of regions or even loose cultural groups the country is associated with. For instance, for "Sweden" it could be something along the way along the way of "Scandinavia", "European Union", "NATO".
- `capitals` This should be a list of names of the country's capital as strings. It should include the official name but also localized names, e.g. "Munich" and "München".
- `flag:` { path, source, license, attributionRequired: boolean }

For the initial version try to refer to https://en.wikipedia.org/wiki/List_of_sovereign_states if possible.
...
More specifally, the normalization for the strings should occur through a normalize(input) function:
	•	Unicode normalize (NFKD),
	•	remove combining marks,
    •   remove diacritics (as explained above),
	•	lowercase,
	•	collapse whitespace,
	•	strip punctuation.
...
Each capital should be rendered in an element that indicates that it might be dragged and drop which should be consistent to the way countries are rendered in the left column as explained above.
